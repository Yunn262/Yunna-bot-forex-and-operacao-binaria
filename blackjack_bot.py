...
def fetch_forex(symbol='EUR/USD', timeframe='15m', limit=200):
    ...
    price = np.cumsum(np.random.randn(limit)*0.1) + 1.1
...
def predict_next_move(df):
    ...
    # Features: open, high, low, close, volume, HL, OC, SMA, SMA_diff
    ...
    confidence = min(max(abs(diff)*1000, 60), 99)  # escala de confiança
